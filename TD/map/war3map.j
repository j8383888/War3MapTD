...
function CreateUnitsForPlayer0 takes nothing returns nothing
    ...
    local trigger t
    local real life
    set u=CreateUnit(p, 'A001', - 27224.7, 9107.8, 14.876)
endfunction
//===========================================================================
...
// Trigger: Skill
//===========================================================================
function Trig_SkillFunc001Func003T takes nothing returns nothing
    local group ydl_group
    local unit ydl_unit
    call SaveInteger(YDLOC, GetHandleId(GetExpiredTimer()), 0x25DAB820, ( ( LoadInteger(YDLOC, GetHandleId(GetExpiredTimer()), 0x25DAB820) ) + ( 1 ) ))
    set ydl_group=CreateGroup()
    call GroupEnumUnitsInRange(ydl_group, GetUnitX(LoadUnitHandle(YDLOC, GetHandleId(GetExpiredTimer()), 0xB6A6EBAA)), GetUnitY(LoadUnitHandle(YDLOC, GetHandleId(GetExpiredTimer()), 0xB6A6EBAA)), S2R(EXExecuteScript("(require'jass.slk').ability[" + I2S('S004') + "].Area1")), null)
    loop
        set ydl_unit=FirstOfGroup(ydl_group)
        exitwhen ydl_unit == null
        call GroupRemoveUnit(ydl_group, ydl_unit)
        if ( ( IsUnitEnemy(ydl_unit, GetOwningPlayer(LoadUnitHandle(YDLOC, GetHandleId(GetExpiredTimer()), 0xB6A6EBAA))) == true ) ) then
            call UnitDamageTarget(LoadUnitHandle(YDLOC, GetHandleId(GetExpiredTimer()), 0xB6A6EBAA), ydl_unit, ( ( I2R(GetHeroInt(LoadUnitHandle(YDLOC, GetHandleId(GetExpiredTimer()), 0xB6A6EBAA), true)) ) * ( 10.00 ) ), true, true, ATTACK_TYPE_NORMAL, DAMAGE_TYPE_NORMAL, WEAPON_TYPE_WHOKNOWS)
        else
        endif
    endloop
    call DestroyGroup(ydl_group)
    if ( ( LoadInteger(YDLOC, GetHandleId(GetExpiredTimer()), 0x25DAB820) == 12 ) ) then
        call RemoveUnit(LoadUnitHandle(YDLOC, GetHandleId(GetExpiredTimer()), 0x1F0BA775))
        call FlushChildHashtable(YDLOC, GetHandleId(GetExpiredTimer()))
        call DestroyTimer(GetExpiredTimer())
    else
    endif
...
function Trig_SkillActions takes nothing returns nothing
    local timer ydl_timer
    local integer ydl_localvar_step= LoadInteger(YDLOC, GetHandleId(GetTriggeringTrigger()), 0xCFDE6C76)
 set ydl_localvar_step=ydl_localvar_step + 3
 call SaveInteger(YDLOC, GetHandleId(GetTriggeringTrigger()), 0xCFDE6C76, ydl_localvar_step)
 call SaveInteger(YDLOC, GetHandleId(GetTriggeringTrigger()), 0xECE825E7, ydl_localvar_step)
    if ( ( GetSpellAbilityId() == 'S004' ) ) then
        ...
        call SaveUnitHandle(YDLOC, GetHandleId(GetTriggeringTrigger()) * ydl_localvar_step, 0xB6A6EBAA, CreateUnitAtLoc(GetOwningPlayer(GetTriggerUnit()), 'e001', LoadLocationHandle(YDLOC, GetHandleId(GetTriggeringTrigger()) * ydl_localvar_step, 0x32A9E4C8), 0))
        set ydl_timer=CreateTimer()
        call SaveInteger(YDLOC, GetHandleId(ydl_timer), 0x25DAB820, 0)
        call SaveUnitHandle(YDLOC, GetHandleId(ydl_timer), 0xB6A6EBAA, GetTriggerUnit())
        call SaveUnitHandle(YDLOC, GetHandleId(ydl_timer), 0x1F0BA775, LoadUnitHandle(YDLOC, GetHandleId(GetTriggeringTrigger()) * ydl_localvar_step, 0xB6A6EBAA))
        call TimerStart(ydl_timer, 0.50, true, function Trig_SkillFunc001Func003T)
...
function InitTrig_Skill takes nothing returns nothing
    ...
    call TriggerAddAction(gg_trg_Skill, function Trig_SkillActions)
endfunction
//===========================================================================
